...
import { useOptiSyncStore } from "@/store";
...
const RenderConnectionAccordion = ({ connection, state }: Props) => {
	...
	const { slackChannels, selectedSlackChannels, setSelectedSlackChannels } = useOptiSyncStore();

	const connectionData = (nodeConnection as any)[connectionKey];

	const isConnected =
		alwaysTrue ||
		(nodeConnection[connectionKey] && accessTokenKey && connectionData[accessTokenKey!]);
